...
def check_valid_input(l_guessed, old_letters_guessed):
  ...
  :type return: str
  """
  heblat = 'אבגדהוזחטיכלמנסעפצקרשתךםןףץ'
  if letter_guessed in heblat and len (letter_guessed) == 1 and letter_guessed not in old_letters:
    check = letter_guessed
  elif letter_guessed in heblat == False and len (letter_guessed) != 1:
    check = ("E1")
  elif letter_guessed in heblat == False and len (letter_guessed) == 1:
    check = ("E2")
  else:
    check = ("E3")
  return (letter_guessed == check)
